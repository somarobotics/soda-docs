...
| **DAgger interventions** | `soda run <policy> --dagger` (implies record, 120 Hz) | ONLY the intervention spans -- one episode each under `recordings/dagger/`; the policy-driven majority of the rollout is not recorded at all (no states, no camera frames). Each episode: executed trajectory + cameras + both command streams, `info.json` marked `dagger: true` + `episode_index`. The episode opens when an arm is grabbed and closes at its A/X hand-back; grabs shorter than 0.3 s are discarded as accidental. Schema 2.1 |
