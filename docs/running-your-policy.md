...
It's all plain HTTP; the host-side `soda` CLI just wraps it (runs on the robot **or any
laptop on the network**, no `docker exec`).
...
**`q` or `Ctrl-C` stops just the policy and leaves the stack up** (so does `s` in `soda
keys`), so you don't need `soda stop`. `soda stop` (identical to `soda estop`) kills every
...
**Hold = `soda pause`.** The arms hold position and the rollout keeps its context; `soda
resume` picks up from wherever the arms are *now*, so you can hand-correct the scene during
...
**Record a rollout (DAgger demo).** Add `--record` to save the whole run — cameras +
...
command. `*_target_*` **defaults from `site.yaml gripper_max_position`** (GR100 open = 0.0 /
close ≈ 0.67), so you usually omit them; the GP100 policy convention is open = 0.0 /
close = 1.52 (GP100 vs GR100: see the [glossary](glossary.md)). **Reversed ranges are fine** — e.g. a UMI parallel jaw in metres: `gripper_source_open: 0.085`,
...
1. Serve your checkpoint as an **openpi-compatible WebSocket server** on your GPU machine.
   Obs/action schema: [`docs/integration/policy-io-contract.md`](shipped-docs.md).
...
**Structural knobs** change the loop shape, so they need a stop → start: `control_hz`,
`send_hz`, `chunk_h`, `reject_outlier`, `sag_comp`, `image_mode`, `action_space`, the
`gripper_*` mapping, `max_steps`. Set them in the entry's `defaults:`, or pass `overrides`
to `soda run` / `POST /policy/start` for a single run.
